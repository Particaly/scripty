...
MVP 优先支持 Windows，并保持执行层可扩展到 macOS 和 Linux。Scripty 不负责安装 Node.js、Python、PowerShell 或 Shell 运行时，请先在本机安装所需解释器。Node 和 Python 的直接依赖在独立“依赖”页维护，并统一安装到 `<userData>/scripty/node_modules` 与 `<userData>/scripty/.venv`；脚本运行不会依赖全局第三方包。解释器会按脚本语言自动查找：先检查 ZTools 宿主进程的 `PATH`，再通过用户登录 Shell（Windows 使用 `where.exe`）补充查找版本管理器提供的运行时。
在 macOS 上，如果图形界面的 `PATH` 中没有 Node，还会兼容检查 `$MISE_DATA_DIR`、`XDG_DATA_HOME/mise` 或 `~/.local/share/mise` 下的标准 Node 安装。

mise 自动发现优先使用 `installs/node/latest/bin/node` 指向的独立 Node 可执行文件；若没有该别名，则兼容回退到 `shims/node`。自动解析出的设备路径只缓存在当前插件进程中，不会写入任务或备份。若未找到解释器，请确认运行时已安装且能从登录终端直接调用。
...
2. 在“脚本”中新建由 Scripty 托管的脚本。
3. 在“依赖”页新增、修改或删除直接依赖后，同步 Node/Python 共享环境；解释器会按脚本语言自动查找。
...
├── src/
│   ├── components/     # 任务、脚本、历史、环境、依赖和备份视图
