...
<template>
  <section class="environments-view">
    ...
    <div class="section-heading"><div><h2>环境变量</h2><p>普通列表不会返回敏感明文。</p><p class="security-notice" role="note">敏感变量以当前设备的本地数据形式保存，不等同于系统密钥库保护。请限制数据目录和导出文件的访问权限。</p></div><div class="section-heading__actions"><ZButton @click="importDotEnv">导入 .env</ZButton><ZButton :disabled="variables.length===0" @click="exportDotEnv">导出 .env</ZButton><ZButton type="primary" @click="createVariable">创建变量</ZButton></div></div>
